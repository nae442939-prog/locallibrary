...
import com.example.locallibrary.repostitory.GenreRepository;
import com.example.locallibrary.repostitory.BookGenreRepository;

// @Component: 빈으로 등록하는 기본 어노테이션
// 데이터베이스 샘플 데이터(Seed)를 생성하는 객체입니다.
// 서버가 실행될 때 자동으로 실행됩니다.
@Component
class SeedDatabase implements CommandLineRunner {
  ...
  public void run(String... args) throws Exception {
    ...
    BookGenre[] bookGenres = {
      new BookGenre(books[0], genres[0]),
      new BookGenre(books[1], genres[0]),
      new BookGenre(books[2], genres[0]),
      new BookGenre(books[3], genres[1]),
      new BookGenre(books[4], genres[1]),
      new BookGenre(books[5], genres[0]),
      new BookGenre(books[5], genres[1]),
      new BookGenre(books[6], genres[0]),
    };

    for (BookGenre bookGenre : bookGenres) {
      log.info("Preloaded book genre: " + bookGenreRepository.save(bookGenre));
    }
    ...
    BookInstance[] bookInstances = {
      ...
      new BookInstance(books[4], "New York, NY Tom Doherty Associates, LLC, 2015.", Status.MAINTENACE, null),
    ...
    log.info("Seeding has been completed.");
  };
}
